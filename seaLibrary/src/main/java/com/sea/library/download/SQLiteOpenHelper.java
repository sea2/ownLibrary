...
public abstract class SQLiteOpenHelper {
    private static final String TAG = "gmdb";
    ...
    public SQLiteDatabase getWritableDatabase(Context context) {
        synchronized (this) {
            return getDatabaseLocked(context,true);
        }
    }
    ...
    public SQLiteDatabase getReadableDatabase(Context context) {
        synchronized (this) {
            return getDatabaseLocked(context,false);
        }
    }

    private SQLiteDatabase getDatabaseLocked(Context context,boolean writable) {
        if (mDatabase != null) {
            if (!mDatabase.isOpen()) {
        ...
        try {
            ...
            if (mName == null) {
                ...
                try {     
                        final String path = getDatabasePath(context,mName).getPath();
                        Log.d(TAG, "path:" + path);
                        db = SQLiteDatabase.openOrCreateDatabase(path, mFactory);
                        Log.d(TAG, "openOrCreateDatabase ok :" + path);
    ...
    public File getDatabasePath(Context mContext,String name)
    {
    	return new File(mContext.getDatabasePath(mName).getPath());
    }
